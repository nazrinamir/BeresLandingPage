...
export default function BenefitSection() {
  ...
  return (
    <section className="relative overflow-hidden rounded-b-2xl flex justify-center py-6 sm:py-8 lg:py-10">
      {/* === GRID BACKGROUND === */}
      <div className="absolute inset-0 bg-[url('/Maskgroupbenefit.svg')] bg-cover bg-no-repeat opacity-90"></div>

      {/* === Top Badge === */}
      <div className="absolute top-4 sm:top-6 flex justify-center w-full z-20">
        <div className="px-4 sm:px-6 py-2 bg-white/10 backdrop-blur-md border !border-black !text-black font-semibold rounded-full text-sm sm:text-lg shadow-sm">
          Benefits
        </div>
      </div>
      ...
      <div className="relative z-10 flex flex-col lg:flex-row items-center lg:items-start justify-between w-[95%] sm:w-[90%] max-w-[1200px] gap-6 sm:gap-8 lg:gap-10 mt-12 sm:mt-14 lg:mt-16">
        ...
        <div className="flex flex-col justify-center lg:justify-start flex-1 text-center lg:text-left lg:pl-4 mt-0 lg:mt-36">
          ...
          <p className="!text-[#012219] text-base sm:text-lg lg:text-[1.3rem] xl:text-[1.5rem] leading-relaxed max-w-md mx-auto lg:mx-0 mb-8 text-justify lg:text-left font-medium">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 lg:gap-8 xl:gap-10 w-full max-w-[400px] sm:max-w-[600px] lg:max-w-[900px] xl:max-w-[980px]">
            {[
              ...
              <motion.div
                ...
                className="bg-white relative rounded-[1.5rem] sm:rounded-[2rem] shadow-[0_8px_25px_rgba(0,0,0,0.08)] flex flex-col items-center justify-center text-center py-6 sm:py-8 lg:py-12 px-4 sm:px-6 lg:px-10"
              >
                {/* Red Grid Overlay */}
                ...
                <h3 className="font-semibold text-[#0B1E18] text-lg sm:text-xl lg:text-2xl mb-4 sm:mb-5 lg:mb-6 relative z-10 leading-snug">
                  {card.title}
                </h3>
                <img
                  src={card.img}
